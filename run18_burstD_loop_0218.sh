#!/bin/bash
# 脚本说明：此脚本会循环 burstSizeD=0,5,10,20,50,70,100,150,200,250,300，为每个 burstSizeD 生成一个独立的输出文件夹，并运行 18 种仿真组合。
...
# 需要循环的 burstSizeD 值
burstSizeD_values=(0 5 10 20 50 70 100 150 200 250 300)
...
# 循环不同的 burstSizeD
for burstSizeD in "${burstSizeD_values[@]}"; do
    BASE_OUTPUT_FOLDER="combine18_burstD${burstSizeD}"
    ...
    for particleSpread in "${particleSpreadOptions[@]}"; do
        for ifnSpread in "${ifnSpreadOptions[@]}"; do
            for dip in "${dipOptions[@]}"; do
                ...
                echo "Executed: ../../${BINARY_FILE} --burstSizeD=${burstSizeD} --particleSpreadOption=${particleSpread} --ifnSpreadOption=${ifnSpread} --dipOption=${dip}"
