#!/bin/bash
# 脚本说明：此脚本会循环 meanLysisTime=3,6,12,18,24，为每个 meanLysisTime 生成一个独立的输出文件夹，并运行 18 种仿真组合。
...
# 需要循环的 meanLysisTime 值
# lysis_values=(3 6 12 18 24)
lysis_values=($(seq 0.5 0.5 12))
...
# 循环不同的 meanLysisTime
for meanLysisTime in "${lysis_values[@]}"; do
    BASE_OUTPUT_FOLDER="combine18_lysis${meanLysisTime}"
    ...
    for particleSpread in "${particleSpreadOptions[@]}"; do
        for ifnSpread in "${ifnSpreadOptions[@]}"; do
            for dip in "${dipOptions[@]}"; do
                ...
                echo "Executed: ../../${BINARY_FILE} --meanLysisTime=${meanLysisTime} --particleSpreadOption=${particleSpread} --ifnSpreadOption=${ifnSpread} --dipOption=${dip}"
