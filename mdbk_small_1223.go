...
const (
	...
	OUTPUT_VIDEO  = "1217.mp4" // Output video file name
				...
				if g.state[i][j] == INFECTED_VIRION || g.state[i][j] == INFECTED_DIP || g.state[i][j] == INFECTED_BOTH {
					...
					if g.state[i][j] == INFECTED_VIRION || g.state[i][j] == INFECTED_DIP {
						...
						if g.state[i][j] == INFECTED_VIRION || g.state[i][j] == INFECTED_BOTH {
							...
							if g.timeSinceInfectVorBoth[i][j] > IFN_DELAY+int(rand.NormFloat64()*float64(STD_IFN_DELAY)) {
						...
						if g.state[i][j] == INFECTED_DIP {
							...
							if g.timeSinceInfectDIP[i][j] > IFN_DELAY+int(rand.NormFloat64()*float64(STD_IFN_DELAY)) {
...
func (g *Grid) recordSimulationData(writer *csv.Writer, frameNum int) {
	...
	row := []string{
		...
		strconv.Itoa(totalDeadFromV),     // Number of cells dead from INFECTED_VIRION
		strconv.Itoa(totalDeadFromBoth),  // Number of cells dead from INFECTED_BOTH
