	...
	if videotype == "states" {
		...
		for i := 0; i < GRID_SIZE; i++ {
			for j := 0; j < GRID_SIZE; j++ {
				...
				if g.state[i][j] == DEAD {
					// For dead cells, use the color of their previous state
					// Add safety check to ensure the previous state exists in colors map
					if prevColor, exists := colors[g.previousStates[i][j]]; exists {
						cellColor = prevColor
					} else {
						// Fallback to gray if previous state is not in colors map
						cellColor = color.RGBA{169, 169, 169, 255}
					}
				} else {
					// For living cells, use their current state color
					// Add safety check to ensure the current state exists in colors map
					if currColor, exists := colors[g.state[i][j]]; exists {
						cellColor = currColor
					} else {
						// Fallback to black if current state is not in colors map
						cellColor = color.RGBA{0, 0, 0, 255}
					}
				}

				drawHexagon(img, x, y, cellColor) // Draw the hexagon based on the cell state
			}
		}
